...
from src.watermark.models import WatermarkRunStatus
...
def test_failure_recovery_and_idempotent_retry():
    """Verify that failure before commit leaves checkpoint unchanged, and retry

    overwrites the landing batch cleanly and commits the checkpoint.
    """
    with tempfile.TemporaryDirectory() as tmpdir:
        base_path = Path(tmpdir)
        db_path = base_path / "control.db"
